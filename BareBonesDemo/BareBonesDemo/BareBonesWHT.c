...
const uint8_t bootEvent[]={0x0F,0x09,0x08,0x00,0x00,0x00,0x01,0x01,0x00,0x00,0x00,0x00};//ya tiene el sync flag

//const uint8_t bootEventP[] PROGMEM={0x0F,0x09,0x08,0x00,0x00,0x00,0x01,0x01,0x00,0x00,0x00,0x00};//ya tiene el sync flag

const uint8_t eventACK[]={0x0F,0x00,0x05,0x00};//

//const uint8_t eventACKP[] PROGMEM={0x0F,0x00,0x05,0x00};//

const uint8_t bootEvent1[]={0x0F,0x09,0x0A,0x00,0x00,0x00,0x01,0x01,0x00,0x00,0x00,0x00};

const uint8_t joinCommand[]={0x06,0x00,0x04};//ignore packetid, request
const uint8_t joinACK[]={0x06,0x00,0x05,0x00};//ignore packet id, response

const uint8_t moteSearchingEvent[]={0x0F,0x09,0x04,0x00,0x00,0x00,0x02,0x02,0x00,0x00,0x00,0x08};
//mandar un eventACK

const uint8_t negotiatingEvent[]={0x0f,0x09,0x04,0x00,0x00,0x01,0x00,0x03,0x00,0x00,0x00,0x08};
//mandar un event ACK

const uint8_t alarmEvent[]={0x0f,0x09,0x04,0x00,0x00,0x00,0x02,0x04,0x00,0x00,0x00,0x00};
//mandar un eventACK
const uint8_t operationalEvent[]={0x0f,0x09,0x02,0x00,0x00,0x00,0x24,0x05,0x00,0x00,0x00,0x00};
//mandar un eventACK
const uint8_t svcIndication[]={0x0e,0x0c,0x04,0x00,0x00,0x80,0x01,0x03,0x02,0xf9,0x81,0x00,0x00,0x03,0xe8};
const uint8_t serviceACK[]={0x0e,0x00,0x05,0x00};


//const uint8_t setService[]={0x01,0x0a,0x05,0x08,0x01,0x01,0x00,0xf9,0x81,0x00,0x00,0x01,0x2c};//request
const uint8_t setService[]={0x01,0x0a,0x05,0x08,serviceID,0x01,0x00,0xf9,0x81,0x00,0x00,0x01,0x2c};//request

const uint8_t setServiceACK[]={0x01,0x02,0x00,0x00,0x08,0x01};

const uint8_t sendComand[]={0x05,0x0d,0x04,0xf9,0x81,0x01,0x00,0x02,0xff,0xff,0xff,0x04,0xb1,0x6b,0x00,0xb5};
const uint8_t sendACK[]={0x05,0x00,0x05,0x00};
/*
05 = send
f9 81 = address
01 = serviceId
...
MoteData_p newMoteData(void){
	MoteData_p moteData=malloc(sizeof(MoteData_t));
	memset(moteData,0,sizeof(MoteData_t));
	return moteData;
}
